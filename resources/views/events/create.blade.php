            ...
            <form method="POST" action="{{ route('events.store') }}"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <select name="venue_id"
                                        @change="updateVenueCapacity(); checkVenueAvailability(); loadVenueLocations()"
                            ...
                            <div x-show="selected_venue_id">
                                ...
                                <div class="space-y-2 mt-2">
                                    <template x-for="location in availableLocations" :key="location.id">
                                        <label class="flex items-start p-3 border border-gray-200 rounded-lg hover:bg-indigo-50 cursor-pointer">
                                            <input type="checkbox"
                                            ...
                                            <div class="ml-3 flex-1">
                                                ...
                                                <p class="text-xs mt-1" :class="location.is_booked ? 'text-red-600' : 'text-gray-600'">
                                                    <span x-show="location.is_booked">
                                                        ❌ Not available for selected dates
                                                    </span>
                                                    <span x-show="!location.is_booked">
                                                        <span x-show="location.amenities">Amenities: <span x-text="location.amenities"></span></span>
                                                    </span>
                                                </p>
                                            </div>
                                        </label>
                                ...
                                <p x-show="selected_venue_location_ids.length === 0" class="mt-2 text-xs text-gray-500">
                                    Select at least one location
                                </p>

                                <div x-show="selected_venue_location_ids.length > 0" class="mt-4 p-3 bg-indigo-50 border border-indigo-200 rounded-lg">
                                    <p class="text-sm font-semibold text-indigo-900">
                                        Total Capacity: <span x-text="location_total_capacity"></span> persons
                                    </p>
                                </div>
                            </div>
                ...
                <div class="px-6 py-4 border-t bg-gray-50 flex items-center justify-end space-x-3">
                    ...
                    <button type="submit"
                            ...
                            class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs font-semibold text-white uppercase tracking-widest hover:bg-indigo-700 transition">
                        Submit Request
                    </button>
                </div>
        ...
        document.addEventListener('alpine:init', () => {
            ...
            Alpine.data('eventForm', (venuesData) => ({
                ...
                get availableLocations() {
                    ...
                    // Mark locations as booked if they have conflicts for selected dates
                ...
                checkLocationBooking(locationId) {
                    // Check if location has a booking during selected time
                    if (!this.start_at || !this.end_at) return false;
                    return this.availability_conflicts.some(conflict => 
                        conflict.venue_location_id === locationId
                    );
                },
                ...
                loadVenueLocations() {
                    // Reload locations after venue selection
                ...
                syncLocationIds() {
                    // Ensure location IDs are sent with the form
